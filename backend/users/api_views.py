...
from .models import User, UserAddress, UserDetail, UserSkill
...
class CurrentUserView(APIView):
    ...
    def get(self, request):
        ...
        # #region agent log
        log_path = r'c:\eldonia-nex\.cursor\debug.log'
        with open(log_path, 'a', encoding='utf-8') as f:
            f.write(json.dumps({'location':'CurrentUserView:get','message':'Returning user data','data':{'user_id':user.id,'subscription':subscription_value,'subscription_plan':getattr(user,'subscription_plan','free')},'timestamp':__import__('time').time()*1000,'sessionId':'debug-session','hypothesisId':'H'}) + '\n')
        # #endregion
...
class ProfileUpdateView(APIView):
    ...
    def patch(self, request):
        ...
        log_path = r'c:\eldonia-nex\.cursor\debug.log'
        with open(log_path, 'a', encoding='utf-8') as f:
            f.write(json.dumps({'location':'ProfileUpdateView:patch','message':'Received profile update','data':{'user_id':user.id,'display_name':display_name,'avatar_url':avatar_url,'email':email},'timestamp':__import__('time').time()*1000,'sessionId':'debug-session','hypothesisId':'H4'}) + '\n')
        # #endregion
        ...
        if update_fields:
            ...
            # #region agent log
            with open(log_path, 'a', encoding='utf-8') as f:
                f.write(json.dumps({'location':'ProfileUpdateView:patch:saved','message':'Saved profile to DB','data':{'update_fields':update_fields,'avatar_url':user.avatar_url},'timestamp':__import__('time').time()*1000,'sessionId':'debug-session','hypothesisId':'H4'}) + '\n')
            # #endregion
...
class SubscriptionUpdateView(APIView):
    ...
    def patch(self, request):
        ...
        log_path = r'c:\eldonia-nex\.cursor\debug.log'
        with open(log_path, 'a', encoding='utf-8') as f:
            f.write(json.dumps({'location':'SubscriptionUpdateView:patch','message':'API called','data':{'user_id':request.user.id,'request_data':request.data},'timestamp':__import__('time').time()*1000,'sessionId':'debug-session','hypothesisId':'F'}) + '\n')
        # #endregion
        ...
        # #region agent log
        with open(log_path, 'a', encoding='utf-8') as f:
            f.write(json.dumps({'location':'SubscriptionUpdateView:patch','message':'DB updated','data':{'old_plan':old_plan,'new_plan':new_plan,'db_subscription':user.subscription,'db_subscription_plan':user.subscription_plan},'timestamp':__import__('time').time()*1000,'sessionId':'debug-session','hypothesisId':'G'}) + '\n')
        # #endregion
...
class UserAddressView(APIView):
    ...
    def patch(self, request):
        ...
        fields = ['country_code', 'country_name', 'postal_code', 'state_province', 
                  'city', 'address_line1', 'address_line2', 'full_address', 
...
class UserSkillDetailView(APIView):
    ...
    def patch(self, request, skill_id):
        ...
        fields = ['skill_name', 'category', 'level', 'years_of_experience', 
                  'certification', 'description', 'sort_order', 'is_primary']
...
class UserDetailView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            return Response({
                ...
                'desired_hourly_rate_min': str(detail.desired_hourly_rate_min) if detail.desired_hourly_rate_min else None,
                'desired_hourly_rate_max': str(detail.desired_hourly_rate_max) if detail.desired_hourly_rate_max else None,
    ...
    def patch(self, request):
        ...
        fields = ['first_name_kanji', 'last_name_kanji', 'first_name_kana', 'last_name_kana',
                  'occupation', 'company_name', 'company_position', 'education', 'education_status',
                  'income_range', 'interests', 'self_introduction', 'desired_work_types',
                  'desired_hourly_rate_min', 'desired_hourly_rate_max', 'available_hours_per_week',
                  'is_remote_available', 'is_identity_verified', 'has_signed_nda', 'notes']
