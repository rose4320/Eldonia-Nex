...
class UserAdmin(DjangoUserAdmin):  # type: ignore
    ...
    def subscription_badge(self, obj):
        colors = {"free": "#6c757d", "basic": "#007bff", "premium": "#28a745", "enterprise": "#ffc107"}
        icons = {"free": "🆓", "basic": "⭐", "premium": "💎", "enterprise": "👑"}
        color = colors.get(obj.subscription, "#6c757d")
        icon = icons.get(obj.subscription, "")
        return format_html(
            (
                '<span style="background-color:{}; color:white; padding:5px 12px; '
                'border-radius:15px; font-weight:600; font-size:11px;">{} {}</span>'
            ),
            color,
            icon,
            obj.subscription.upper(),
        )
    ...
    def level_badge(self, obj):
        return format_html(
            (
                ...
                'color:white; padding:5px 12px; border-radius:15px; font-weight:600;">Lv.{}</span>'
                '<br><small style="color:#6c757d;">{} XP</small>'
            ),
            obj.current_level,
            obj.total_exp,
        )
        ...
        if obj.is_superuser:
            return format_html(
                (
                    ...
                    'border-radius:10px; font-size:10px;">🛡️ スーパー</span>'
                )
            )
        elif obj.is_staff:
            return format_html(
                (
                    ...
                    'border-radius:10px; font-size:10px;">👤 スタッフ</span>'
                )
            )
        return "-"
...
class UserProfileAdmin(admin.ModelAdmin):  # type: ignore
    ...
    def user_display(self, obj):
        return format_html("<strong>{}</strong><br><small>{}</small>", obj.user.username, obj.user.email)
...
class PlanAdmin(admin.ModelAdmin):  # type: ignore
    ...
    fieldsets = (
        ("📋 基本情報", {"fields": ("name", "slug", "tier")}),
        ("💰 価格設定", {"fields": ("price", "currency", "billing_cycle")}),
        ("✨ 機能・特典", {"fields": ("features", "trial_days")}),
        ("⚙️ 表示設定", {"fields": ("is_active", "sort_order")}),
        ("📊 統計", {"fields": ("subscriber_count_detail",), "classes": ("collapse",)}),
    ...
    def trial_badge(self, obj):
        if obj.trial_days > 0:
            return format_html(
                (
                    '<span style="background:#17a2b8; color:white; padding:3px 8px; '
                    'border-radius:10px; font-size:11px;">🎁 {}日間</span>'
                ),
                obj.trial_days,
            )
        return "-"
