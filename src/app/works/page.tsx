...
export default async function WorksPage({ searchParams }: WorksPageProps) {
  ...
  const jobs = await getJobListings({ q, type });
    ...
    <div className="eldonia-page">
      ...
      <main className="mx-auto flex w-full max-w-7xl flex-1 flex-col gap-8 px-6 py-8">
        <section className="space-y-2">
          ...
          <h1 className="eldonia-heading eldonia-heading-lg">Guild of Creators</h1>
          <p className="eldonia-body text-sm">
            求人・協業マッチング。応募時にポートフォリオ（EXP / Lv / 称号）を自動添付。
          </p>
        </section>
        ...
        <nav className="flex flex-wrap gap-2">
          {[{ value: "all", label: "すべて" }, ...JOB_TYPES].map((item) => {
        ...
        <section className="grid gap-4 lg:grid-cols-2">
          {jobs.length === 0 ? (
            <p className="eldonia-body col-span-full py-16 text-center">
              該当する求人がありません。
            </p>
          ) : (
