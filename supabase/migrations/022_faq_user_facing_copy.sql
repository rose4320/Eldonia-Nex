...
update public.support_faq_articles
set
  answer = '作品の「公開」設定がオンになっているかご確認ください。アップロード直後は反映に数秒かかる場合があります。それでも表示されない場合は、対応しているファイル形式か、ファイルサイズが上限以内かをご確認ください。解決しない場合はお問い合わせフォームから「GALLERY（作品）」カテゴリでご連絡ください。',
  updated_at = now()
